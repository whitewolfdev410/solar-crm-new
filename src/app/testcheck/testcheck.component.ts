...
@Component({
  ...
  styleUrls: ['./testcheck.component.css']
})
export class TestcheckComponent implements OnInit {
  ...
  constructor(public http: HttpClient, public fb: UntypedFormBuilder, private router: Router, private route: ActivatedRoute, @Inject(DOCUMENT) private document: any, private preventivi: PreventiviService, private Location: Location, public dialog: MatDialog, private service: ContattiService) { }
  ...
  ListPreventivi(): void {
    this.loading = true;
    this.preventivi.getPreventiviDev()
      .subscribe(response => {
